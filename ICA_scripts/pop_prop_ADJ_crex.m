...
function com = pop_prop_ADJ(EEG, typecomp, numcompo, winhandle, is_H, is_V, is_B, is_D,...
    soglia_DV, diff_var, soglia_K, med2_K, meanK, soglia_SED, med2_SED, SED, soglia_SAD, med2_SAD, SAD, ...
        soglia_GDSF, med2_GDSF, GDSF, soglia_V, med2_V, maxvar, soglia_D, maxdin,activations) %,spec_opt)
...
%                        '', ei_smooth, 1, 'caxis', 2/3, 'cbar','erp'); 
         ...
         icaacttmp = (EEG.icaweights(numcompo,:) * EEG.icasphere) ...
                                       * reshape(EEG.data(1:size(EEG.icaweights,1),:,:), EEG.nbchan-8, EEG.trials*EEG.pnts);
    %                     icaacttmp = (EEG.icaweights(numcompo,:) * EEG.icasphere) ...
...
%                          EI_TITLE, ei_smooth, 1, 'caxis', 2/3, 'cbar');
       ...
       offset = nan_mean(activations(numcompo,:));
                    erpimage( ...
              reshape(activations(numcompo,1:erpimageframestot),erpimageframes,ERPIMAGELINES)-offset, ...
...
%                                      * reshape(EEG.data, erpimageframes, ERPIMAGELINES);
                        ...
                        icaacttmp = EEG.icaweights(numcompo,:) * EEG.icasphere ...
                                     *EEG.data(1:size(EEG.icaweights,1),1:erpimageframes*ERPIMAGELINES);
...
% try
	...
	if typecomp == 1
        ...
        [spectra, freqs] = spectopo( activations(numcompo,:), EEG.pnts, EEG.srate, 'freqrange', [0 45] );
	else 
		if option_computeica  
            disp('--------option computeica---------------');
...
%             end
            [spectra, freqs] = spectopo(EEG.icaact, EEG.pnts, EEG.srate, 'mapnorm', EEG.icawinv(:,numcompo),  'freqrange', [0 45]);  % EEG.icaact(numcompo,:)
% 		else
% 			if exist('icaacttmp')~=1, 
%                 
% 				icaacttmp = (EEG.icaweights(numcompo,:)*EEG.icasphere)*reshape(EEG.data, EEG.nbchan-8, EEG.trials*EEG.pnts); 
% 			end;
%             
